// Nano#2 LCD Control, SD card & coordination w #1

// Modified from Demo_Landscape example on RinkyDinkElectronics.com
#include <UTFT.h>
#include <tinyFAT.h>
...
UTFT LCD = UTFT(ITDB32S,38,39,40,41); //TFT01_70, 38, 39, 40, 41); //will need to change pins for our LCD
...
//******************************************************************//
Setup(){
  if(sizeof(layerIMG) > 1){ //initializes SEQ_LAYER to say that next layer exists. Will stay high until all layers complete.
    digitalWrite(SEQ_LAYER,HIGH);
  }
...
//******************************************************************//
void ISR_1(){ //listens for next command from Nano1 
  digitalWrite(SEQ_FDBK, HIGH);
  i=i+1; //increments image index
  }

//******************************************************************//
ISR_2(){} //does nothing right now.
